-- Utility for Peachtree fleet demo reset (telematics_events is append-only)
...
COMMENT ON FUNCTION public.reset_fleet_demo_tenant(uuid) IS
  'Clears fleet demo data for a tenant; used by scripts/seed-fleet-demo/reset.ts';

REVOKE ALL ON FUNCTION public.reset_fleet_demo_tenant(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.reset_fleet_demo_tenant(uuid) TO service_role;
